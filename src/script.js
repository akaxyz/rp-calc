...
function recalcAll() {
    ...
    for (let tier in tierData) {
        ...
        if (stationFormula[tier]) {
            ...
            // SDC: multiplier + bonus rp
            results[tier] = { rps: rpSum, cap: null, isSDC: true, capFactor: capFactorSum };
        }
    }
    ...
    let totalRPS = 0;
    for (let tier in results) {
        if (!results[tier].isSDC) totalRPS += results[tier].rps;
    }

    // apply SDC multiplier if present
    if (results["SDC"] && results["SDC"].isSDC) {
        const sdcBonus = results["SDC"].rps;
        const sdcMult  = results["SDC"].capFactor;
        totalRPS = totalRPS * sdcMult + sdcBonus;
    }

    document.getElementById("totals").textContent = "Total RP/s: " + totalRPS.toFixed(2);
...
function renderTierResults(results) {
    ...
    for (let tier in results) {
        const { rps, cap, isSDC, capFactor } = results[tier];
        const row = document.createElement("div");
        row.className = "tier-row";
        ...
        if (isSDC) {
            const mult  = (capFactor  || 0).toFixed(2);
            const bonus = (rps        || 0).toFixed(2);
            row.innerHTML = `
                <span class="tier-badge">SDC</span>
                <span class="col stat-bonus">+${bonus}</span>
                <span class="col stat-none">—</span>
                <span class="col stat-mult">×${mult}</span>
            `;
        } else {
